...
export const Calendar = ({selected: undefined, onChange}:CalendarProps) => {
    ...
    const getDayClassName = useCallback(
        ...
        return [
            ...
            selectedDate && format(dt, 'yyyyMMdd') === format(selectedDate, 'yyyyMMdd') && 'selected'
        ].join(' ')
    ...
    return (
        <div className="calendar">
            <div className="month">
                <button type="button" className="btn-today" onClick={()=>{
            ...
            <ul className="days">
                {dates.map((dt, index)=> 
                <li key={index} className={getDayClassName(dt)} onClick={()=>{setSelectedDate(dt)}}>
                    {format(dt, 'd')}
                </li>) }
